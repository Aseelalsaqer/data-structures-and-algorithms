...
const sortByChildren = (charArray) => {
  // Solution code here...
  charArray.sort((a,b)=> a.children.length === b.children.length ? a.house > b.house?1:a.house < b.house ?-1:0  : a.children.length - b.children.length);
  return charArray;
};
...
const citiesAtoJ = (arr) => {
  // Solution code here...
  var regex= /^[A-J](\w)*/g;
  return arr.filter(a=>a.match(regex));
};
...
const matchMonth = (input) => {
  // Solution code here...
  return input.match && input.match(/^[Oo]ct(ober){0,1}$/g) !== null;
};
...
const noPunctuation = str => {
  // Solution code here...
  return [...str.matchAll(/\w+\s{1}/g)].map(a => a[0]);
};
...
const findShells = (str) => {
  // Solution code here...
  return [...str.matchAll(/\w*ells/g)].map(a => a[0]);
};
